...
public class JoyStickManager : MonoBehaviour
{
    ...
    private void HandleFingerMove(Finger movedFinger)
    {
        if (movedFinger == MovementFinger)
        {
            ...
            if (Vector2.Distance(
                ...
                Joystick.joyStickObjAnchoredPosition
            ) > maxMovement)
            {
                knobPosition = (
                                   currentTouch.screenPosition - Joystick.joyStickObjAnchoredPosition
                               ).normalized
                               * maxMovement;
            }
            else
            {
                knobPosition = currentTouch.screenPosition - Joystick.joyStickObjAnchoredPosition;
            }
    ...
    private void HandleFingerDown(Finger touchedFinger)
    {
        if (MovementFinger == null && touchedFinger.screenPosition.x <= Screen.width)
        {
            ...
            Joystick.joyStickObjSizeDelta = JoystickSize;
            Joystick.joyStickObjAnchoredPosition = ClampStartPosition(touchedFinger.screenPosition);
        }
    }
